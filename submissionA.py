...
def main():
    ...
    train_x["keyword"] = tf.convert_to_tensor(train_df["keyword"], )
    val_x["keyword"] = tf.convert_to_tensor(val_df["keyword"], )
    test_x["keyword"] = tf.convert_to_tensor(test_df["keyword"], )
    train_y = tf.convert_to_tensor(train_df["target"])
    val_y = tf.convert_to_tensor(val_df["target"])

    strategy = tf.distribute.get_strategy()
    with strategy.scope():
        bert_inputs = [
            tf.keras.Input(name=name, shape=(tokens, ), dtype=tf.int32)
            for name in tokenizer.model_input_names
        ]
        inputs = bert_inputs + [
            tf.keras.Input(name="keyword", shape=(1, ), dtype=tf.int32)
        ]
        assert bert_config.num_labels == 2
        bert_model = TFBertForSequenceClassification(config=bert_config)
        bert_output = bert_model(bert_inputs)
        model = tf.keras.Model(inputs=inputs, outputs=[bert_output])
        model.summary()
