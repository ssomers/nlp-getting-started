...
submit = True
batch_size = 12
train_ratio = .8 if submit else .01
val_ratio = .2 if submit else .01
stopping_accuracy = .99
max_epochs = 50
optimizer = tf.keras.optimizers.Adam(1e-5)
regularizer = tf.keras.regularizers.L2(5)
...
def bert_column_to_tf(col, tokenizer):
    return tf.ragged.constant([tokenizer.encode(s) for s in col])
...
def bert_encode_text(df, tokenizer):
    ...
    return {
        "input_ids": text,
        "attention_mask": tf.ones_like(text),
...
def train(model, log_dir, train_x, train_y, val_x, val_y):
    ...
    output.kernel_regularizer = regularizer
...
def main():
    ...
    with strategy.scope():
        ...
        train(model=model,
              ...
              val_y=val_y)
    if submit:
        logits = model.predict(test_x).logits
        target = np.argmax(logits, axis=1)
        submission = test_df[["id"]].assign(target=target)
